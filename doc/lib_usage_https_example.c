...
int main(int argc, char *argv[])
{
    ...
    *(void **)(&dbc_close_captcha_fn) = load_symbol(lib, "dbc_close_captcha");
    ...
    if (dbc_set_transport_fn(client, DBC_TRANSPORT_HTTPS)) {
        fprintf(stderr, "Failed selecting HTTPS transport. Ensure build has libcurl support.\n");
        dbc_close_fn(client);
        free(client);
#ifdef _WIN32
        FreeLibrary(lib);
    ...
    printf("[https] User ID: %lu\n[https] Balance: %.3f US cents\n", client->user_id, client->balance);
    ...
    if (DBC_SAMPLE_TOKEN_NONE != token_mode) {
        ...
        if (!rc && NULL != captcha) {
            save_outputs("https", captcha);
        } else {
            fprintf(stderr, "Failed solving token-based captcha in HTTPS sample\n");
        }
